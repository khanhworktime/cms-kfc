import React, {EventHandler, FormEvent, useEffect, useReducer, useState} from 'react';
import Navbar from "../../../components/Navbar/navbar";
import {
    DataGrid,
    GridColDef,
    GridSelectionModel,
    GridSlotsComponentsProps, GridToolbarFilterButton, GridToolbarQuickFilter,
                            ...
                            <Autocomplete
                                ...
                                renderInput={(params) => (
                                    <TextField margin={"normal"}
                                               ...
                                               label="Role" />)}
                            />
                            <FormControlLabel control={<Checkbox checked={userFormInput.activate}  onChange={handleInput} name={"activate"} id={"activate"} />} label="Activate this user ?" />
                        </Box>
                        <div className={"flex gap-4 justify-end"}>
                            {userFormInput.method =="post" && <Button onClick={createNewUserHandler} variant={"contained"}>Add</Button>}
                            {userFormInput.method =="put" && <Button onClick={updateUserHandler} variant={"contained"}>Update</Button>}
                            <Button variant={"contained"} onClick={()=>setUserFormInput({...reducerInit})} color={"error"}>Cancel</Button>
                        </div>
                    </CusBox>
                </div>
             </Fade>
